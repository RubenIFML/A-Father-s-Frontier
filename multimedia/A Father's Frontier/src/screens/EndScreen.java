...
public class EndScreen extends BScreen {
    ...
    public EndScreen(Demo game) {
        ...
        periodicoFinalActor.addAction(Actions.sequence(
                Actions.fadeIn(4f), // Fade in del periodoFinalActor durante 1 segundo
                Actions.run(() -> isClickable = true) // Permitir interacción después del fade in
        ));
        ...
        uiStage.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                if (isClickable) {
                    periodicoFinalActor.addAction(Actions.sequence(
                            Actions.fadeOut(3f), // Fade out del periodoFinalActor durante 1 segundo
                            Actions.delay(3f), // Retraso de 3 segundos después del fade out
                            Actions.run(() -> {
                                finalisimoActor.addAction(Actions.sequence(
                                        Actions.fadeIn(3f), // Fade in del finalisimoActor durante 1 segundo
                                        Actions.delay(6f), // Retraso de 6 segundos después del fade in
                                        Actions.fadeOut(4f), // Fade out del finalisimoActor durante 1 segundo
                                        Actions.delay(3f), // Retraso de 6 segundos después del fade in
